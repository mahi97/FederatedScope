...
def extend_data_cfg(cfg):
    ...
    cfg.data.root = os.environ.get(
        'HF_DATASETS_CACHE',
        os.environ.get(
            'DATA_DIR',
            os.path.join(_storage, 'data') if _storage else 'data',
        ),
    )
    cfg.data.type = 'toy'
    ...
    # Fast GLUE benchmark knobs
    cfg.data.glue = CN()
    cfg.data.glue.train_size = -1
    cfg.data.glue.val_size = -1
    cfg.data.glue.test_size = -1
    cfg.data.glue.sample_seed = 42
