...
class Monitor(object):
    ...
    def __init__(self, cfg, monitored_object=None):
        ...
        self.active_round_timing = dict()
        self.round_timing_records = defaultdict(list)
    ...
    def finish_fl(self):
        ...
        """
        if self.active_round_timing:
            logger.warning("Discard unfinished round timers: %s",
                           list(self.active_round_timing.keys()))
            self.active_round_timing.clear()
    ...
    def get_sys_metrics(self, verbose=True):
        ...
        system_metrics.update(self.get_round_timing_summary_metrics())
        if verbose:
            logger.info(
    ...
    def start_round_timing(self, timer_name, rnd):
        self.active_round_timing[timer_name] = {
            "round": rnd,
            "start_time": time.perf_counter()
        }

    def stop_round_timing(self, timer_name, role, rnd=None):
        timer_info = self.active_round_timing.pop(timer_name, None)
        if timer_info is None:
            return None

        duration = time.perf_counter() - timer_info["start_time"]
        self.track_round_timing(timer_name,
                                duration=duration,
                                rnd=timer_info["round"] if rnd is None else rnd,
                                role=role)
        return duration

    def cancel_round_timing(self, timer_name):
        self.active_round_timing.pop(timer_name, None)

    def track_round_timing(self, timer_name, duration, rnd, role):
        duration = float(duration)
        self.round_timing_records[timer_name].append({
            "round": rnd,
            "duration": duration
        })
        formatted_timing_res = {
            "Role": role,
            "Round": rnd,
            "Results_raw": {
                f"timing/{timer_name}_seconds": duration
            }
        }
        logger.info(formatted_timing_res)
        self.save_formatted_results(formatted_timing_res,
                                    save_file_name="round_timing.log")
        self._update_round_timing_summary(timer_name, role)

    def get_round_timing_summary_metrics(self):
        timing_summary_metrics = dict()
        for timer_name, records in self.round_timing_records.items():
            if not records:
                continue

            values = [record["duration"] for record in records]
            avg_values = values[1:] if len(values) > 1 else values
            timing_summary_metrics[
                f"{timer_name}_avg_seconds_excluding_first_round"] = \
                float(np.mean(avg_values))
            timing_summary_metrics[
                f"{timer_name}_timed_rounds"] = len(values)
            timing_summary_metrics[
                f"{timer_name}_first_round_skipped_in_average"] = \
                len(values) > 1
        return timing_summary_metrics

    def _update_round_timing_summary(self, timer_name, role):
        ...
        timing_summary_metrics = self.get_round_timing_summary_metrics()
        for key, value in timing_summary_metrics.items():
            if key.startswith(f"{timer_name}_"):
                wandb.summary[f"{role}, timing_summary/{key}"] = value
    ...
    def save_formatted_results(self,
        ...
        if self.use_wandb and self.wandb_online_track:
            try:
                ...
                round_id = formatted_res.get("Round")
                if isinstance(round_id, (int, np.integer)):
                    wandb.log(log_res, step=int(round_id))
                else:
                    wandb.log(log_res)
            except ImportError:
                logger.error(
    ...
    def finish_fed_runner(self, fl_mode=None):
        ...
        if self.use_wandb and not self.wandb_online_track:
            ...
            with open(os.path.join(self.outdir, "eval_results.log"),
                ...
                round_timing_f_name = os.path.join(self.outdir,
                                                   "round_timing.log")
                if os.path.exists(round_timing_f_name):
                    exp_stop_normal = False
                    with open(round_timing_f_name, "r") as f:
                        for line in f:
                            exp_stop_normal, log_res = logline_2_wandb_dict(
                                exp_stop_normal,
                                line,
