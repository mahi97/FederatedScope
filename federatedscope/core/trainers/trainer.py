...
class Trainer(BaseTrainer):
    ...
    def evaluate(self, target_data_split_name="test", hooks_set=None):
        ...
        mode = MODE.TEST if target_data_split_name == "test" else MODE.VAL
    ...
    def _param_filter(self, state_dict, filter_keywords=None):
        ...
        if self.cfg.personalization.share_non_trainable_para:
            trainable_filter = lambda name: True
        else:
            trainable_filter = lambda name: name in \
                self.ctx.trainable_para_names
