...
class Client(BaseClient):
    ...
    def _calculate_W(self, updated_model):
        model = self.trainer.ctx.model.state_dict()
        for key in updated_model:
            A, B = None, None
            if 'lora_A' in key:
                A = updated_model[key]
                B = model[key.replace('lora_A', 'lora_B')]
                w = torch.norm(B, p=2, dim=0)
                avg_w = torch.mean(w)
                w = (w + 1e-5) / (avg_w + 1e-5) if avg_w > 1e-5 else torch.ones_like(w)
                W = torch.diag(w)
                A = W @ A
            # print('Model: ', model[key.replace('lora_A', 'lora_B')].shape)
            # print(updated_model[key].shape)
            updated_model[key] = A
    ...
    def callback_funcs_for_model_para(self, message: Message):
        ...
        if 'ss' in message.msg_type:
                ...
                self.comm_manager.send(
                    Message(msg_type='model_para',
                            ...
                            content=(sample_size, first_aggregate_model_para[0]
                                     ...
                                     first_aggregate_model_para)))
                self._monitor.stop_round_timing(
                    timer_name='client_round_time',
            ...
            self._monitor.start_round_timing(timer_name='client_round_time',
                                             rnd=round)
            ...
            if self._cfg.federate.use_ss:
                ...
                if self._cfg.asyn.use or self._cfg.aggregator.robust_rule in \
                    ...
                    # model_para_all = self._calculate_W(model_para_all)
    ...
    def callback_funcs_for_finish(self, message: Message):
        ...
        self._monitor.cancel_round_timing('client_round_time')
