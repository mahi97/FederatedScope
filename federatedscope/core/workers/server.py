...
class Server(BaseServer):
    ...
    def __init__(self,
        ...
        # For LLMs with adapters and shared_local_model, avoid deepcopy to save memory
        # The model is shared and only adapter parameters are trained
        if self._cfg.federate.share_local_model and self._cfg.llm.adapter.use:
            self.models = [self.model]  # Use reference instead of deepcopy
        else:
            ...
            while self.state <= self.total_round_num:
                try:
                    ...
                    if not move_on_flag and not move_on_flag_eval:
                        ...
                        self._monitor.cancel_round_timing('server_round_time')
    ...
    def _start_new_training_round(self, aggregated_num=0):
        ...
        if self._cfg.asyn.use:  # for asynchronous training
            ...
            self.broadcast_model_para(msg_type='model_para',
                                      sample_client_num=self.sample_client_num)
        self._monitor.stop_round_timing(timer_name='server_round_time',
                                        role='Server #')
    ...
    def eval(self):
        ...
        if self._cfg.federate.make_global_eval:
            ...
            for i in range(self.model_num):
                ...
                if update_best_this_round and \
                        self._cfg.federate.save_to != '' and \
                        self.ds_rank == 0:
                    self.aggregator.save_model(
                        self._cfg.federate.save_to, self.state)
            self.check_and_save()
        else:
    ...
    def callback_funcs_model_para(self, message: Message):
        ...
        if round == self.state and \
                'server_round_time' not in self._monitor.active_round_timing:
            self._monitor.start_round_timing(timer_name='server_round_time',
                                             rnd=round)
