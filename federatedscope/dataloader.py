...
def load_glue_dataset(config=None, **kwargs):
    ...
    # Determine cache directory for model/tokenizer
    model_cache_dir = config.llm.cache.model if config.llm.cache.model else None
    
    # load tokenizer - always try local_files_only first if cache exists
    try:
        tokenizer = AutoTokenizer.from_pretrained(
            model_name,
            cache_dir=model_cache_dir,
            local_files_only=True,  # Always try local first
        )
    except OSError:
        # Fall back to downloading if local files not found and not in offline mode
        if OFFLINE_MODE:
            raise OSError(
                f"Cannot load tokenizer for '{model_name}' in offline mode. "
