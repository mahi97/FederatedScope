...
class MAPATrainer(GeneralTorchTrainer):
    ...
    def _hook_on_fit_start_init(self, ctx):
        ...
        for name, param in ctx.model.named_parameters():
            if param is not None and ctx.cfg.personalization.local_param[0] not in name:
        ...
        d = G.numel()
        k = d // ctx.cfg.mapa_rank  # Ensure that ctx.k is defined.
        # print('All:  ', d)
        remainder = d % k
        s = (d + k - remainder) // k
    ...
    def _hook_on_batch_backward(self, ctx):
        ...
        grad_list = []
        for name, param in ctx.model.named_parameters():
            if param.grad is not None and ctx.cfg.personalization.local_param[0] not in name: # remove personalization
        ...
        GG_matrix = ctx.A @ (ctx.A.T @ G_matrix)
        ...
        pointer = 0
        for name, param in ctx.model.named_parameters():
            if param.grad is not None and ctx.cfg.personalization.local_param[0] not in name:
                numel = param.grad.numel()
                new_grad = GG[pointer:pointer + numel].view_as(param.grad)
