...
def get_tokenizer(model_name, cache_dir, tok_len=128, pkg='huggingface_llm'):
    ...
    # Use cache_dir only if it's non-empty
...
def load_llm_dataset(config=None, **kwargs):
    ...
    if dataset_name.endswith('.json'):
        ...
        gsm8k_cfg = config.llm.gsm8k

        # Split off a fixed validation subset for fast-eval
        rng = random.Random(gsm8k_cfg.sample_seed)
        val_size = max(0, min(gsm8k_cfg.val_size, len(list_data_dict)))
        indices = list(range(len(list_data_dict)))
        rng.shuffle(indices)
        val_indices = indices[:val_size]
        train_indices = indices[val_size:]
        if gsm8k_cfg.train_size > 0:
            train_indices = train_indices[:gsm8k_cfg.train_size]
        ...
        if not os.path.exists(fp_test):
            ...
            # The downloaded file is named test.jsonl; move it to the expected
            # gsm8k_test.jsonl so subsequent runs do not re-download.
            os.rename(os.path.join(config.data.root, 'test.jsonl'), fp_test)
        list_data_dict_test = load_jsonl(fp_test,
                                         instruction='question',
                                         output='answer')
        if gsm8k_cfg.test_size > 0:
            test_rng = random.Random(gsm8k_cfg.sample_seed + 1)
            test_indices = list(range(len(list_data_dict_test)))
            test_rng.shuffle(test_indices)
            test_indices = test_indices[:gsm8k_cfg.test_size]
            list_data_dict_test = [list_data_dict_test[i]
