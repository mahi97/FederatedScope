#!/bin/bash
# Run all GLUE rebuttal baselines (personalized lora_B) in parallel, one per GPU

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPORT_FILE="$SCRIPT_DIR/failure_report.log"

# Clear previous report
echo "GLUE Rebuttal (personalized lora_B) Report - Started: $(date)" > "$REPORT_FILE"
echo "" >> "$REPORT_FILE"

echo "Launching svd_pB on GPU 0..."
bash "$SCRIPT_DIR/run_svd.sh" 0 &
PID_SVD=$!

echo "Launching svd_no_gram_pB on GPU 1..."
bash "$SCRIPT_DIR/run_svd_no_gram.sh" 1 &
PID_SVD_NO_GRAM=$!

echo "Launching full_rank_pB on GPU 2..."
bash "$SCRIPT_DIR/run_full_rank.sh" 2 &
PID_FULL_RANK=$!

echo "Launching fedma_pB on GPU 3..."
bash "$SCRIPT_DIR/run_fedma.sh" 3 &
PID_FEDMA=$!

echo "Launching fa_lora_pB on GPU 4..."
bash "$SCRIPT_DIR/run_fa_lora.sh" 4 &
PID_FA_LORA=$!

echo "Launching factor_fedprox_pB on GPU 5..."
bash "$SCRIPT_DIR/run_factor_fedprox.sh" 5 &
PID_FACTOR_FEDPROX=$!

echo "Launching factor_fedavg_pB on GPU 6..."
...
echo ""
echo "All baselines (personalized lora_B) finished at $(date)"
