...
METHOD_TO_CFG = {
    "factor_fedavg":
    REPO_ROOT /
    "federatedscope/glue/yamls/wallclock/roberta_large_factor_fedavg.yaml",
    "factor_fedprox":
    REPO_ROOT /
    "federatedscope/glue/yamls/wallclock/roberta_large_factor_fedprox.yaml",
    "fa_lora":
    REPO_ROOT /
    "federatedscope/glue/yamls/wallclock/roberta_large_fa_lora.yaml",
    "svd":
    REPO_ROOT / "federatedscope/glue/yamls/wallclock/roberta_large_svd.yaml",
    "svd_no_gram":
    REPO_ROOT /
    "federatedscope/glue/yamls/wallclock/roberta_large_svd_no_gram.yaml",
    "fedma":
    REPO_ROOT / "federatedscope/glue/yamls/wallclock/roberta_large_fedma.yaml",
    "full_rank":
    REPO_ROOT /
    "federatedscope/glue/yamls/wallclock/roberta_large_full_rank.yaml",
}
...
TASK_META = {
    "cola": {
        ...
        "best_key": "val_matthews_correlation",
        "matched": False,
    },
    "mnli": {
        "metrics": ["accuracy"],
...
SCENARIO_LOCAL_PARAMS = {
    "shared_b": ["classifier"],
    "personalized_b": ["classifier", "lora_B"],
}

INVALID_COMBOS = {
    ("fa_lora", "personalized_b"): "freeze_A baseline requires shared lora_B",
    ("full_rank",
     "personalized_b"): "full-rank aggregation requires shared lora_B",
}
...
def ensure_results_header(results_path: Path, reset=False):
    header = (
        ...
        "`scripts/run_glue_wallclock_matrix.py`.\n\n"
        "| Task | Method | Scenario | Status | Server Round Avg (s) | "
        "Client Round Avg Mean (s) | Client Round Avg Std (s) | "
        "Server FL End (min) | Client FL End Mean (min) | "
...
def summarize_metrics(task, method, scenario, server, clients, notes=""):
    client_round_values = [
        client.get("client_round_time_avg_seconds_excluding_first_round")
        for client in clients if client.get(
            "client_round_time_avg_seconds_excluding_first_round") is not None
    ]
    client_timed_rounds = [
        client.get("client_round_time_timed_rounds")
        for client in clients if client.get("client_round_time_timed_rounds")
        is not None
    ]
    client_fl_end = [
        client.get("fl_end_time_minutes")
        for client in clients if client.get("fl_end_time_minutes") is not None
    ]
    ...
    return {
        ...
        "server_round_avg":
        format_float(
            server.get("server_round_time_avg_seconds_excluding_first_round")),
        "client_round_avg_mean":
        format_float(mean(client_round_values)),
        "client_round_avg_std":
        format_float(std(client_round_values)),
        "server_fl_end_min":
        format_float(server.get("fl_end_time_minutes")),
        "client_fl_end_mean":
        format_float(mean(client_fl_end)),
        "server_timed_rounds":
        str(server.get("server_round_time_timed_rounds", "N/A")),
        "client_timed_rounds_mean":
        format_float(mean(client_timed_rounds)),
        "notes":
        notes or "`ok`",
    }


def append_nonrun_row(results_path: Path,
                      task,
                      method,
                      scenario,
                      status,
    ...
    append_markdown_row(
        results_path, {
            ...
            "server_fl_end_min": "N/A",
            "client_fl_end_mean": "N/A",
...
def build_case_command(gpu, task, method, scenario, outdir, save_to, model_type):
    meta = TASK_META[task]
    ...
    return [
        ...
        str(SCENARIO_LOCAL_PARAMS[scenario]),
        "eval.metrics",
        str(meta["metrics"]),
...
def main():
    ...
    parser.add_argument("--root-outdir",
                        default=str(OUTDIR_ROOT),
                        help="Root directory for experiment outputs")
    parser.add_argument("--root-save-dir",
                        default=str(CKPT_ROOT),
                        help="Root directory for checkpoints")
    parser.add_argument("--model-type",
                        default="FacebookAI/roberta-large@huggingface_llm")
    ...
    tasks = parse_csv_arg(args.tasks, TASK_ORDER)
        ...
        for method in methods:
            for scenario in scenarios:
                ...
                if invalid_reason is not None:
                    append_nonrun_row(results_path, task, method, scenario,
                                      "N/A", invalid_reason)
                    continue

                case_name = f"{task}__{method}__{scenario}"
                outdir = outdir_root / task / scenario / method
                ...
                cmd = build_case_command(args.gpu, task, method, scenario,
                                         outdir, save_to, args.model_type)
                if args.dry_run:
                    print(" ".join(cmd))
                    continue

                print(
                    f"[start] task={task} method={method} scenario={scenario} "
                ...
                if proc.returncode != 0:
                    print(
                        f"[failed] task={task} method={method} "
                        f"scenario={scenario} exit={proc.returncode} "
                        f"log={log_path}",
                        flush=True)
                    append_nonrun_row(results_path, task, method, scenario,
                                      "failed",
                ...
                try:
                    ...
                    print(
                        f"[done] task={task} method={method} scenario={scenario} "
                    ...
                    print(
                        f"[parse_failed] task={task} method={method} "
                        f"scenario={scenario} error={type(error).__name__}: {error}",
                        flush=True)
                    append_nonrun_row(results_path, task, method, scenario,
                                      "parse_failed",
